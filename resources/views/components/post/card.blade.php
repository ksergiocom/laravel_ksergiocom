<div class="sm:rounded-xl sm:shadow-xl overflow-hidden sm:border border-b border-gray-200 flex flex-col">
    <a href="{{ route('post.show', ['slug' => $post->slug]) }}">
        @if($post->imagen)
            <img class="object-cover sm:border-b border-gray-200" 
                 src="{{ Storage::url($post->imagen?->imagen) }}"
                 alt="{{ $post->titulo }}" 
    ...
    <div class="p-5 flex flex-col gap-2 flex-grow">
                ...
                @foreach ($post->tecnologias as $tecnologia)
                    <li class="-ml-4">
                        <img class="h-10 w-10 rounded-full shadow-xl border border-gray-300 bg-white"
                             src="{{ Storage::url($tecnologia->imagen) }}" 
        ...
        <p class="tracking-tight text-gray-600 flex-grow">{{ $post->descripcion }}</p>

        <a class="text-red-500 hover:text-red-600 inline-block tracking-wider hover:underline underline-offset-2 mt-5"
           href="{{ route('post.show', ['slug' => $post->slug]) }}">
           Seguir leyendo
        </a>
    </div>
